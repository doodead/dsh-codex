/**
 * Adapted from @earendil-works/pi-ai 0.82.1 at commit
 * b4f293684bba718d59cc1157679bcf6157b3a7f5 (MIT).
 * Local changes route raw Responses events through the structured observer.
 */
...
import { processStructuredResponsesStream as processResponsesStream } from "./process-structured-responses.ts";
import {
	combineAbortSignals,
	formatProviderError,
	headersToRecord,
...
function buildRequestBody(
	...
	const body: RequestBody = {
		...
		...(cacheSessionId === undefined ? {} : { prompt_cache_key: cacheSessionId }),
	...
	for await (const event of events) {
		...
		if (type === "response.failed") {
			...
			throw new CodexApiError(message || "Codex response failed", {
				...(code === undefined ? {} : { code }), payload: event,
			});
		}
...
async function processWebSocketStream(
	...
	try {
		...
		if (options?.signal?.aborted) {
			...
			const replayItems = expandCodexReplayMarkers(convertResponsesMessages(model, { messages: [withCodexPiReplayMarkers(output)] }, CODEX_TOOL_CALL_PROVIDERS, {
				includeSystemPrompt: false,
				grammarToolInputProperties,
			})) as ResponseInput;
			const responseItems = replayItems.filter(
...
async function parseErrorResponse(response: Response): Promise<{ message: string; friendlyMessage?: string }> {
	...
	return { message, ...(friendlyMessage === undefined ? {} : { friendlyMessage }) };
}
...
function extractAccountId(token: string): string {
	try {
		...
		if (encoded === undefined) throw new Error("Invalid token");
